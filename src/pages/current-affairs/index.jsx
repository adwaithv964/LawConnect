...
import { getCurrentAffairs } from '../../utils/api';
    ...
    return (
        ...
        >
            <div className="relative h-44 overflow-hidden bg-muted flex-shrink-0">
                {item.image && !imgError ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-warning/10 to-warning/5">
                        <Icon name="Newspaper" size={40} color="var(--color-warning)" />
                    </div>
                )}
                {item.source && (
            ...
            <div className="flex flex-col gap-2 p-4 flex-1">
                ...
                {item.snippet && (
                    <p className="text-xs text-muted-foreground line-clamp-2 flex-1">{item.snippet}</p>
                )}
...
const CurrentAffairs = () => {
    ...
    const fetchNews = useCallback(async (cat = selectedCategory) => {
        ...
        setError(null);
        try {
            const data = await getCurrentAffairs(cat, 24);
    ...
    return (
        <div className="min-h-screen bg-background">
            <Helmet>
                <title>Current Affairs – LawConnect</title>
                <meta name="description" content="Stay updated with the latest Indian and world current affairs — politics, economy, science, and more." />
            </Helmet>
            ...
            <main className="mx-4 lg:mx-6 py-6 lg:py-8">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
                    <div>
                        <div className="flex items-center gap-2 mb-1">
                            ...
                            <span className="text-xs font-semibold text-warning uppercase tracking-widest">Live Feed</span>
                        </div>
                        <h1 className="text-3xl lg:text-4xl font-heading font-bold text-foreground mb-1">
                            Current Affairs
                        </h1>
                        <p className="text-muted-foreground text-sm">
                            Latest Indian and world news — politics, economy, science, and more.
                        </p>
                    </div>
                ...
                <p className="text-center text-xs text-muted-foreground mt-10">
                    News sourced from The Hindu, Times of India, NDTV, and Indian Express. Click any card to read the full article.
                </p>
            </main>
        </div>
    );
